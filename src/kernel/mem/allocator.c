...
void idmap_kheap()
{
    ...
    pte.R = 1;
    pte.W = 1;
    id_map_range(kmem_head(), kmem_head() + (NUM_BLOCKS * PAGE_SIZE), pte.bit_repr);
}
...
void allocator_init()
{
    ...
    ASSERT(kmem_alloc != NULL, "allocator_init(): Unable to allocate pages for kernel memory");
...
{
    return block_alloc(BLOCKS_FOR_BYTE_CNT(align(sz)));
}
...
void *block_alloc(size_t block_cnt)
{
    ...
    if (current == NULL)
    {
        kprintf("No blocks big enough to hold 0x%x bytes. Allocating %d pages instead. \n", block_cnt, PAGES_FOR_BLOCK_CNT(block_cnt));
        return pgalloc_alloc(PAGES_FOR_BLOCK_CNT(block_cnt));
    }

    if (current->size > block_cnt)
    {
        kprintf("Found a block that was too large. Splitting into two blocks of size %d and %d\n", current->size - block_cnt, block_cnt);
...
/// @brief Frees the block at the given pointer, and coalesces into other blocks if possible.
...
void block_free(void *b)
{
    ...
    while (current->next != NULL &&
           ...
           (current + current->size * BLOCK_SIZE) == current->next)
    {
        ASSERT(current->size == 0, "CURRENT HAS SIZE OF ZERO");
        // Coalesce with next
...
/// @brief Aligns a byte size variable upwards so it is optimized for memory access
/// @param n Requested byte size
/// @return Optimal byte count
inline size_t align(size_t n)
{
    return (n + sizeof(__WORDSIZE) - 1) & ~(sizeof(__WORDSIZE) - 1);
}