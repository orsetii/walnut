...
fn load_kernel(boot_services: &BootServices) -> (ElfFile, KernelEntry) {
    ...
    let file_handle = unsafe { root
        .open(cstr16!(r"EFI\kernel\kernel.elf"),
    ...
    let len: usize = file_handle.read(buf).expect("Failed to read file");
